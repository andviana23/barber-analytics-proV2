...
- **Frontend:** Next.js 15 (App Router) + React 19 + MUI 5 + TanStack Query
...
export function getDayPilotTheme(mode: 'light' | 'dark') {
  ...
  return {
    css: `
      ...
      .scheduler_cell {
        border-right: 1px solid ${isLight ? tokens.borderLight : tokens.borderDarkLight};
        border-bottom: 1px solid ${isLight ? tokens.borderLight : tokens.borderDarkLight};
      }
      ...
      .scheduler_header {
        ...
        border-bottom: 1px solid ${isLight ? tokens.borderDefault : tokens.borderDark};
        color: ${isLight ? tokens.textPrimary : tokens.textPrimaryDark};
      }
      ...
      .scheduler_event {
        ...
        color: white;
        border-radius: 6px;
        box-shadow: ${isLight ? '0 1px 3px rgba(59, 130, 246, 0.2)' : '0 2px 8px rgba(96, 165, 250, 0.3)'};
      }

      .scheduler_event:hover {
        box-shadow: ${isLight ? '0 4px 12px rgba(59, 130, 246, 0.3)' : '0 6px 16px rgba(96, 165, 250, 0.4)'};
      }
...
export function Scheduler({ tenantId, resources }: SchedulerProps) {
  ...
  const updateEventMutation = useMutation({
    mutationFn: (event: DayPilot.Event) => updateSchedulerEvent(tenantId, event),
    onSuccess: () => {
  ...
  return (
    <div style={{ width: '100%', height: '600px' }}>
      <DayPilot.Scheduler
        ...
        rowHeaderColumns={[
          { property: 'name', text: 'Barbeiro', width: 200 },
        ]}
...
<form>
  ...
  <Button type="submit" tabIndex={4}>Salvar</Button>
  ...
  return (
    <Box
      sx={{
        ...
        transition: theme.palette.mode === 'light'
          ? 'all 160ms cubic-bezier(0.4, 0, 0.2, 1)'
          : 'all 160ms cubic-bezier(0.4, 0, 0.2, 1)',
      }}
    >
      <label htmlFor={`my-component-${label}`}>
        {label}
      </label>
      <input
      ...
      />
      {error && (
        <span role="alert" style={{ color: tokens.colorError, fontSize: '12px' }}>
          Erro ocorreu
        </span>
      )}
    </Box>
...
function ThemeDebugger() {
  ...
  return (
    <Box sx={{ p: 2, backgroundColor: '#000', color: '#0f0', fontFamily: 'monospace', fontSize: '12px', maxHeight: '300px', overflow: 'auto' }}>
      <pre>
        {JSON.stringify({
